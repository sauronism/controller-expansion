...
#define LED_CHANNELS_OFFSET     22
#define LED_STRIPS              4
#define LED_COLOR_CHANNELS      3
#define LED_CHANNELS            (LED_STRIPS * LED_COLOR_CHANNELS)

#define DMX_PADDING            (LED_CHANNELS_OFFSET - BEAM_MAX_CHANNELS - 1)
//#define DMX_MAX_CHANNELS         (BEAM_MAX_CHANNELS + DMX_PADDING + LED_CHANNELS)
#define DMX_MAX_CHANNELS         512 //(BEAM_MAX_CHANNELS + DMX_PADDING + LED_CHANNELS)
...
typedef union DmxBuffer {
  ...
  struct {
    BeamState beam;
    byte padding; // assuming 20 channel beam state for padding
    LedState leds;
  };
  byte buffer[DMX_MAX_CHANNELS];
} DmxBuffer;
static_assert(sizeof(BeamState) == BEAM_MAX_CHANNELS, "BeamState size mismatch");
//static_assert(sizeof(DmxBuffer) == DMX_MAX_CHANNELS, "DmxBuffer size mismatch");
...
namespace sauronism {
  ...
  namespace led_control {
    ...
    void set_led_color(int strip, rgb color) {
      dmx_buffer.leds.color[strip] = color;
    }
...
//        delay(50);
      ...
      Serial.println(dmx_buffer.buffer[0]);
    }

//    void run_led_sequence() {
//      for (int i = 0; i < LED_STRIPS; i++) {
//        set_led_color(i, colors::RED);
//        for (int j = 0; j < 256; j++) {
//          dmx_buffer.leds.color[i].r = j;
//        }
//        set_led_color(i, colors::BLACK);
//      }
//    }
  }
...
void loop() {
  ...
  EVERY_N_MILLISECONDS(100) {
    sauronism::led_control::run_led_sequence();
  }
